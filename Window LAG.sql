...
SELECT name, DAY(whn),
CASE
	WHEN LAG(confirmed, 1) OVER (ORDER BY whn) IS NULL THEN NULL
    ELSE confirmed - LAG(confirmed, 1) OVER (ORDER BY whn)
    END AS result
FROM covid
WHERE name = 'Italy'
AND MONTH(whn) = 3
...
CASE
	WHEN LAG(confirmed, 1) OVER (PARTITION BY name ORDER BY DATE_FORMAT(whn,'%Y-%m-%d')) IS NULL
	THEN NULL
	ELSE (confirmed - LAG(confirmed, 1) OVER (PARTITION BY name ORDER BY DATE_FORMAT(whn,'%Y-%m-%d')))
	END AS new
FROM covid
WHERE name = 'Italy'
AND WEEKDAY(whn) = 0
...
SELECT world.name, ROUND(100000 * confirmed / population, 0),
	RANK() OVER (ORDER BY ROUND(10000 * confirmed / population, 0))
FROM covid
JOIN world
...
FROM
(SELECT *, RANK() OVER (PARTITION BY name ORDER BY new DESC) AS rank
FROM
(SELECT name, DATE_FORMAT(whn, '%Y-%m-%d') AS date,
CASE
	WHEN LAG(confirmed, 1) OVER (PARTITION BY name ORDER BY whn) IS NULL THEN 0
    ELSE confirmed - LAG(confirmed, 1) OVER (PARTITION BY name ORDER BY whn)
    END AS new
FROM covid
ORDER BY name, whn) AS one
WHERE new > 1000) AS two
WHERE rank = 1
ORDER BY date, name
